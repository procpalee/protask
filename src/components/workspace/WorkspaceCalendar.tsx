...
/** 워크스페이스 월간 캘린더 — 상위 Calendar와 동일 디자인(시작전/백로그 + 프로젝트 태그), 칩은 프로젝트색 */
export default function WorkspaceCalendar({ tasks, projects }: { tasks: Task[]; projects: Project[] }) {
  ...
  const colorOf = (t: Task) => projectColor(t.project_id, projects)
        ...
        <div className="flex min-h-0 flex-1 gap-3">
          <div className="flex min-w-0 flex-1 flex-col">
            ...
            <div className="grid flex-1 auto-rows-fr grid-cols-7 overflow-y-auto">
              {days.map(d => {
                const ds = toStr(d)
                return <DayCell key={ds} date={d} dateStr={ds} inMonth={isSameMonth(d, anchor)} isToday={ds === today} tasks={byDate.get(ds) ?? []} colorOf={colorOf} onOpen={openDetail} />
              })}
            </div>
          </div>
...
function DayCell({ date, dateStr, inMonth, isToday, tasks, colorOf, onOpen }: {
  date: Date; dateStr: string; inMonth: boolean; isToday: boolean; tasks: Task[]; colorOf: (t: Task) => string; onOpen: (id: string) => void
}) {
  ...
  return (
    ...
    >
      ...
      <div className="min-h-0 flex-1 space-y-0.5 overflow-y-auto">
        {tasks.slice(0, MAX).map(t => <CalChip key={t.id} task={t} color={colorOf(t)} onOpen={onOpen} />)}
...
/* ───── 사이드 패널 (시작전 + 백로그) — 상위 Calendar와 동일 ───── */
